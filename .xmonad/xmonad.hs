...
myConfigs = [ ("bashrc", myEditor ++ "~/.bashrc", "the bourne again shell")
            , ("VIM", myEditor ++ "~/.config/nvim/lua/custom/init.lua", "VIM config init")
            , ("xmonad.hs", myEditor ++ "~/.xmonad/xmonad.hs", "xmonad config")
            , ("zshrc", myEditor ++ "~/.zshrc", "config for the z shell")
            , ("redshift", myEditor ++ "~/.config/redshift.conf", "config for redshift")
            , ("alacritty.yml", myEditor ++ "~/.config/alacritty/alacritty.yml", "config for alacritty")
            , ("xmobarrc", myEditor ++ "~/.config/xmobar/xmobarrc", "config for xmobar")
            ]
...
myKeys =
    ...
    -- Tree Select/
        ...
        , ("C-s a", treeselectAction tsDefaultConfig)
        -- tree select workspaces menu
        , ("C-s t", TS.treeselectWorkspace tsDefaultConfig myWorkspaces W.greedyView)
        -- tree select choose workspace to send window
        , ("C-s g", TS.treeselectWorkspace tsDefaultConfig myWorkspaces W.shift)
    ...
    -- Scratchpads
        , ("M-S-<Down>", namedScratchpadAction myScratchPads "terminal")
        , ("M-S-p", namedScratchpadAction myScratchPads "spotify")
        , ("M-S-m", namedScratchpadAction myScratchPads "slack")
        -- , ("M-S-p", namedScratchpadAction myScratchPads "calculator")
        -- , ("M-s-c", namedScratchpadAction myScratchPads "mocp")
    ...
    --- My Applications (Super+Alt+Key)
        ...
        , ("M-M1-b", spawn "firefox www.youtube.com/c/DistroTube/")
